...
import {
  buildNorwayNorgesBankYieldSnapshot,
  snapshotToRowViews,
} from "./fetchNorwayNorgesBankCurve";
import { getNorwayNorgesBankYieldHistory } from "./norwayYieldCurve.server";
import { isNorwayHistoryCacheFresh, NO_HISTORY_CACHE_TTL_MS } from "./norwayHistoryCache";

const LOG = "[NO_CURVE]";
const BROWSER_LS_KEY = "market-pulse:no-norgesbank-yield-history:v1";

export type NorwayHistoryModel = GetNorwayYieldHistoryResponse;

export type NorwayNorgesBankYieldCurveResult = {
  snapshot: YieldCurveSnapshot;
  rows: YieldCurveRowView[];
  dataSourceTag: GetNorwayYieldHistoryResponse["dataSourceTag"];
...
function persistBrowserHistory(history: ParsedNorwayNorgesBankHistory): void {
...
function loadBrowserHistory(): { savedAt: number; history: ParsedNorwayNorgesBankHistory } | null {
  ...
  try {
    ...
    const obj = parsed as { savedAt?: unknown; history?: ParsedNorwayNorgesBankHistory };
...
function browserCacheToHistoryModel(
  br: { savedAt: number; history: ParsedNorwayNorgesBankHistory },
): NorwayHistoryModel {
  return {
    history: br.history,
...
function buildCurveResult(
  historyModel: NorwayHistoryModel,
  comparison: YieldComparisonId,
): NorwayNorgesBankYieldCurveResult | null {
  const history = historyModel.history;
  if (!history?.series?.length) return null;

  const snapshot = buildNorwayNorgesBankYieldSnapshot(
    history,
    comparison,
  ...
  const isDisk = tag === "norgesbank-disk-cache";
  ...
  return {
    ...
    fallbackHint: isBrowser
      ? `Using cached Norges Bank data (${new Date(historyModel.cacheSavedAtISO ?? historyModel.updatedAtISO).toLocaleString()})`
      : isDisk
        ? "Using cached Norges Bank data (server disk)"
        : null,
...
async function fetchNorwayHistoryModel(
  serverFn: (input: { data: { forceRefresh?: boolean } }) => Promise<GetNorwayYieldHistoryResponse>,
  forceRefresh = false,
): Promise<NorwayHistoryModel | null> {
  try {
...
function initialHistoryFromBrowser(): NorwayHistoryModel | undefined {
  const br = loadBrowserHistory();
  if (!br) return undefined;
  return browserCacheToHistoryModel(br);
}

/** Yield Curves (Norway) — history fetched once per TTL; comparisons rebuilt locally. */
export function useNorwayNorgesBankYieldCurve(comparison: YieldComparisonId, enabled = true) {
  const serverFn = useServerFn(getNorwayNorgesBankYieldHistory);

  const historyQuery = useQuery({
    queryKey: ["yield-curve-no-norgesbank-history"],
    queryFn: async () => {
      const live = await fetchNorwayHistoryModel(serverFn, false);
    ...
    enabled,
    staleTime: NO_HISTORY_CACHE_TTL_MS,
    ...
    refetchOnMount: (query) => {
      ...
      return !isNorwayHistoryCacheFresh(updatedAt);
    },
  });
