import { useMemo } from "react";
import { useQuery } from "@tanstack/react-query";
import { useServerFn } from "@tanstack/react-start";
import type {
  GetChinaYieldHistoryResponse,
  ParsedChinaChinaBondHistory,
  YieldComparisonId,
  YieldCurveRowView,
  YieldCurveSnapshot,
} from "./types";
import {
  buildChinaChinaBondYieldSnapshot,
  snapshotToRowViews,
} from "./fetchChinaChinaBondCurve";
import { getChinaChinaBondYieldHistory } from "./chinaYieldCurve.server";
import { isChinaHistoryCacheFresh, CN_HISTORY_CACHE_TTL_MS } from "./chinaHistoryCache";

const LOG = "[CN_CURVE]";
const BROWSER_LS_KEY = "market-pulse:cn-chinabond-yield-history:v1";

export type ChinaHistoryModel = GetChinaYieldHistoryResponse;

export type ChinaChinaBondYieldCurveResult = {
  snapshot: YieldCurveSnapshot;
  rows: YieldCurveRowView[];
  dataSourceTag: GetChinaYieldHistoryResponse["dataSourceTag"];
  serverErrorMessage: string | null;
  fallbackHint: string | null;
  cacheSavedAtISO: string | null;
  historyFetchedAt: string | null;
};

function persistBrowserHistory(history: ParsedChinaChinaBondHistory): void {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(
      BROWSER_LS_KEY,
      JSON.stringify({ savedAt: Date.now(), history }),
    );
  } catch {
    // ignore
  }
}

function loadBrowserHistory(): { savedAt: number; history: ParsedChinaChinaBondHistory } | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(BROWSER_LS_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as unknown;
    if (!parsed || typeof parsed !== "object") return null;
    const obj = parsed as { savedAt?: unknown; history?: ParsedChinaChinaBondHistory };
    if (!obj.history?.series?.length) return null;
    const savedAt = typeof obj.savedAt === "number" ? obj.savedAt : Date.now();
    return { savedAt, history: obj.history };
  } catch {
    return null;
  }
}

function browserCacheToHistoryModel(
  br: { savedAt: number; history: ParsedChinaChinaBondHistory },
): ChinaHistoryModel {
  return {
    history: br.history,
    dataSourceTag: "browser-local-storage",
    errorMessage: null,
    updatedAtISO: new Date(br.savedAt).toISOString(),
    cacheSavedAtISO: new Date(br.savedAt).toISOString(),
  };
}

function buildCurveResult(
  historyModel: ChinaHistoryModel,
  comparison: YieldComparisonId,
): ChinaChinaBondYieldCurveResult | null {
  const history = historyModel.history;
  if (!history?.series?.length) return null;

  const snapshot = buildChinaChinaBondYieldSnapshot(
    history,
    comparison,
    historyModel.updatedAtISO,
  );
  if (!snapshot) return null;

  const tag = historyModel.dataSourceTag;
  const isBrowser = tag === "browser-local-storage";
  const isDisk = tag === "chinabond-disk-cache";

  return {
    snapshot,
    rows: snapshotToRowViews(snapshot),
    dataSourceTag: tag,
    serverErrorMessage: historyModel.errorMessage,
    fallbackHint: isBrowser
      ? `Using cached ChinaBond data (${new Date(historyModel.cacheSavedAtISO ?? historyModel.updatedAtISO).toLocaleString()})`
      : isDisk
        ? "Using cached ChinaBond data (server disk)"
        : null,
    cacheSavedAtISO: historyModel.cacheSavedAtISO,
    historyFetchedAt: history.fetchedAt,
  };
}

async function fetchChinaHistoryModel(
  serverFn: (input: { data: { forceRefresh?: boolean } }) => Promise<GetChinaYieldHistoryResponse>,
  forceRefresh = false,
): Promise<ChinaHistoryModel | null> {
  try {
    const payload = await serverFn({ data: { forceRefresh } });
    if (payload.history?.series?.length) {
      persistBrowserHistory(payload.history);
      return payload;
    }
    return payload.history ? payload : null;
  } catch (e) {
    console.info(`${LOG} Server history fetch failed — ${e instanceof Error ? e.message : String(e)}`);
    return null;
  }
}

function initialHistoryFromBrowser(): ChinaHistoryModel | undefined {
  const br = loadBrowserHistory();
  if (!br) return undefined;
  return browserCacheToHistoryModel(br);
}

/** Yield Curves (China) — history fetched once per TTL; comparisons rebuilt locally. */
export function useChinaChinaBondYieldCurve(comparison: YieldComparisonId, enabled = true) {
  const serverFn = useServerFn(getChinaChinaBondYieldHistory);

  const historyQuery = useQuery({
    queryKey: ["yield-curve-cn-chinabond-history"],
    queryFn: async () => {
      const live = await fetchChinaHistoryModel(serverFn, false);
      if (live?.history?.series?.length) return live;

      const br = loadBrowserHistory();
      if (br?.history?.series?.length) {
        console.info(`${LOG} Loaded from browser localStorage after server miss`);
        return browserCacheToHistoryModel(br);
      }
      return live;
    },
    enabled,
    staleTime: CN_HISTORY_CACHE_TTL_MS,
    gcTime: 24 * 60 * 60 * 1000,
    initialData: initialHistoryFromBrowser,
    initialDataUpdatedAt: () => loadBrowserHistory()?.savedAt,
    placeholderData: (prev) => prev,
    refetchOnWindowFocus: false,
    refetchOnMount: (query) => {
      const updatedAt = query.state.dataUpdatedAt;
      if (!updatedAt) return true;
      return !isChinaHistoryCacheFresh(updatedAt);
    },
  });

  const data = useMemo(() => {
    if (!historyQuery.data?.history?.series?.length) return null;
    return buildCurveResult(historyQuery.data, comparison);
  }, [historyQuery.data, comparison]);

  const isUpdating = historyQuery.isFetching && Boolean(historyQuery.data?.history?.series?.length);

  return {
    ...historyQuery,
    data,
    isPending: historyQuery.isPending && !data,
    isUpdating,
  };
}
